...
void Tetris::keyPressEvent(QKeyEvent *event) {
  ...
  UserAction_t action = ACTION;  // Действие по умолчанию
  ...
  // Применяем действие и обновляем отображение
  sigact(action, &currentFigure, &field);
  update();
}
...
void Tetris::safeCheckField() {
  if (field.screen != nullptr) {
    // Создаем глубокую копию поля
    Field tempField;
    tempField.x_end = field.x_end;
    tempField.y_end = field.y_end;
    tempField.x_start = field.x_start;
    tempField.y_start = field.y_start;
    tempField.game_is_on = field.game_is_on;
    tempField.score = field.score;
    tempField.next = field.next;
    tempField.pause = field.pause;
    tempField.speed = field.speed;
    tempField.level = field.level;
    tempField.high_score = field.high_score;

    // Выделяем память и копируем экран
    tempField.screen = (int **)calloc(field.y_end, sizeof(int *));
    for (int i = 0; i < field.y_end; i++) {
      tempField.screen[i] = (int *)calloc(field.x_end, sizeof(int));
      for (int j = 0; j < field.x_end; j++) {
        tempField.screen[i][j] = field.screen[i][j];
      }
    }

    // Выполняем проверку
    check_fild(&tempField);

    // Если проверка прошла успешно, копируем результат обратно
    if (tempField.screen != nullptr) {
      for (int i = 0; i < field.y_end; i++) {
        for (int j = 0; j < field.x_end; j++) {
          field.screen[i][j] = tempField.screen[i][j];
        }
      }
      field.score = tempField.score;
      field.level = tempField.level;
      field.speed = tempField.speed;
      field.high_score = tempField.high_score;
    }

    // Очищаем временное поле
    for (int i = 0; i < tempField.y_end; i++) {
      free(tempField.screen[i]);
    }
    free(tempField.screen);
  }
}

void Tetris::fixedCheckField(Field *field) {
  int cnt = 0;
  int from = PADDING + 1;
  int to = (2 + BOARD_X);
  ...
  while (pointer > 2) {
    int flag = 1;
    // Проверяем строку на заполнение
    ...
    if (flag) {
      // Строка заполнена - пропускаем её и увеличиваем счетчик
      pointer -= 1;
      cnt += 1;
      continue;
    } else if (pointer == current) {
      // Текущая строка не заполнена и не требует копирования
      pointer -= 1;
      current -= 1;
    } else {
      // Копируем строку с pointer в current
      for (int i = from; i < to; i++) {
  ...
  for (int row = 2; row <= current; row++) {
    for (int col = from; col < to; col++) {
      field->screen[row][col] = 0;
    }
  }

  // Обновляем счет и настройки
